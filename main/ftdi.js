...
export class FtdiBridge {
  ...
  async mpsseEnterI2cMode() {
    await this.rawWrite(
      new Uint8Array([
        ...
        MpsseCommands.DRIVE_ZERO_FOR_I2C,
        0x07,
        0x00, // on AD0, 1 and 2
        MpsseCommands.DISABLE_LOOP_BACK,
        MpsseCommands.SET_TCK_SK_DIV,
        0xc8,
        0x00, // 0xC8 divider
      ]).buffer
    )
  ...
  async mpsseSetAC(bits = []) {
    ...
    await this.rawWrite(new Uint8Array([MpsseCommands.SET_DATA_BITS_HIGH, outHighs, outFlags]).buffer)
  }

  async i2cSetLinesIdle() {
    await this.rawWrite(
      new Uint8Array([
        MpsseCommands.SET_DATA_BITS_LOW,
        0xff,
        0xfb, // set all lines to high, all output except input pin 2
      ]).buffer
    )
  }
  ...
  async i2cStart() {
    ...
    for (let i = 0; i < 4; i++) {
      // repeat command to ensure timing
    ...
    if (position !== bytes.byteLength) throw new FtdiBridgeError('L0g1C')
  ...
  async i2cSendByte(value) {
    await this.rawWrite(
      new Uint8Array([
        MpsseCommands.DATA_OUT_BYTES_NEG_EDGE,
        0x00,
        0x00,
        value, // out ONE byte
        MpsseCommands.SET_DATA_BITS_LOW,
        0xfe,
        0xfb, // CLK -> LOW, DATA -> HIGH
        MpsseCommands.DATA_IN_BITS_POS_EDGE,
        0x00, // in ONE bit
  ...
  async i2cTransferPacket(address, outputData, inputLength) {
    ...
    try {
      if (outputData) {
        ...
        if (!addressAck) throw new FtdiBridgeError(`Address ${address} did not acknowledge send packet start`)

        const bytes = new Uint8Array(outputData)
        for (let i = 0; i < bytes.length; i++) {
          const dataAck = await this.i2cSendByte(bytes[i])
          if (!dataAck) throw new FtdiBridgeError(`Address ${address} did not acknowledge reception of byte ${i}`)
        }
      }
      ...
      if (inputLength) {
        ...
        if (!addressAck) throw new FtdiBridgeError(`Address ${address} did not acknowledge receive packet start`)
